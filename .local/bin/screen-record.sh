...
WF_RECORDER_OPTS_ALT_1="--audio=$defaultSink.monitor -c libx264"
WF_RECORDER_OPTS_ALT_2="--audio=$defaultSink.monitor -c libx264 -p crf=23 -p preset=medium"
outputFile=""
outputDir=""
...
stopRecording() {
    ...
    action=$(notify-send "Recording stopped" "Saved to: $outputPath" \
        -i video-x-generic \
        -a "Screen Recorder" \
        -t 10000 \
        --action="open_dir=Open Directory" \
        --action="play_file=Play")

    case "$action" in
      open_dir)
      xdg-open "$(dirname "$outputPath")"
      ;;
    play_file)
      xdg-open "$outputPath"
      ;;
...
startRecording() {
    ...
    if [ "$target" == "screen" ]; then
        ...
        if [ -z "$quality" ]; then
            ...
            yad --title="Choose an Option" \
                --text="Please select an option:" \
                --button="High Quality":1 \
                --button="Medium Quality":2 \
                --button="Low Quality":3 \
                --center
            ...
            1)
                ;;
            2)
                WF_RECORDER_OPTS="$WF_RECORDER_OPTS_ALT_1"
                ;;
            3)
            WF_RECORDER_OPTS="$WF_RECORDER_OPTS_ALT_2"
                ;;
            *)
                echo "No option selected or dialog was closed."
                exit 1
                ;;
            esac
    ...
    outputDir="${outputDir:-$HOME/Videos/Recordings}"
